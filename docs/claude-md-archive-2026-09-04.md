# CLAUDE.md as it stood on 2026-09-04 (archived in full)

This is the complete text of `CLAUDE.md` immediately before it was cut from
32KB to under 8KB on 2026-09-04. Nothing here was deleted from the project —
it was moved out of the file that is read into **every session**, because at
32KB it cost roughly 8,200 tokens per session before a single word was typed.

The rules survive in the new `CLAUDE.md` as one-liners. The reasoning, the
incident narratives and the worked examples live here. Read this when you need
to know *why* a rule exists; the rule itself is in `CLAUDE.md`.
