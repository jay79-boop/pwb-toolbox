---
status: active
project: personal
type: log
created: {{date:YYYY-MM-DD}}
---

<!-- This template lives in your vault at `01 - Daily Notes/Daily Note Template.md`. Every daily note gets created from a copy of it. -->

# {{date:dddd, MMMM D, YYYY}}

## Index
<!-- One bullet per session/topic. Bold the topic, then a one-sentence past-tense outcome. Update this BEFORE adding session body content below. -->

- **Session topic name** — one-line summary of what happened / what was decided

<!-- Timestamp every session heading with the local time (e.g. 1:26 PM), never UTC. -->
## Session 1 — {{time:h:mm A}}: {{topic}}

### What Got Done
-

### What's Still In Progress
-

### Decisions Made
-

### Notes Touched
<!-- Wikilinks to any vault notes created, edited, or referenced this session -->
-

### Profile Updates
<!-- If the AI updated any profile sections of the vault index this session, list what changed -->
-

---
