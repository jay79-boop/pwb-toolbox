...
def _load_dataset_from_pwb(
    dataset_name: str, split: str, pwb_api_key: str, symbols=None
) -> pd.DataFrame:
...
def _load_dataset_from_hf(
    dataset_name: str, split: str, hf_token: str, symbols=None
) -> pd.DataFrame:
    ...
    return _read_parquet_files(
        local_files, desc=f"Load {dataset_name}", symbols=symbols
    )
...
def load_dataset(
    ...
    # This supersedes an earlier in-place expansion of `symbols`, which reached
    # the same result by mutating the caller's list.
    symbol_filter = None
    ...
    if pwb_api_key and not use_hf:
        df = _load_dataset_from_pwb(
            path, split=split, pwb_api_key=pwb_api_key, symbols=symbol_filter
        )
    else:
        hf_token = os.getenv("HF_ACCESS_TOKEN")
        if hf_token:
            df = _load_dataset_from_hf(
                path, split=split, hf_token=hf_token, symbols=symbol_filter
            )
        elif path in YFINANCE_FALLBACK_DATASETS:
            # The free fallback takes the expanded filter too, so `sp500` works
            # here exactly as it does on the paid paths.
            df = _load_dataset_from_yfinance(path, symbol_filter)
        else:
