...
def run_strategy(config: StrategyConfig):
    ...
    Args:
        config: StrategyConfig with all strategy, data, and backtrader settings.

    Returns:
        Backtrader strategy instance with log_data.
    """
    cerebro_kwargs = config.cerebro_kwargs or {}
    broker_kwargs = config.broker_kwargs or {}
    ...
    cerebro.addstrategy(
        ...
        indicator_cls=config.indicator_cls,
        indicator_kwargs=config.indicator_kwargs,
    ...
    if "commission" not in broker_kwargs:
        commission = np.mean(list(pwb_bt.get_commissions(config.symbols).values()))
