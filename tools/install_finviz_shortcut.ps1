<#
.SYNOPSIS
  Put a "Finviz Research" icon on the Desktop that starts it in one
  double-click. Run this once.

.DESCRIPTION
  Same pattern as hermes-agent's tools\karaoke_server\install_shortcut.ps1,
  adapted for this repo. The icon points at powershell.exe with
  '-ExecutionPolicy Bypass -File', not at the .ps1 directly. Two reasons,
  and both have already bitten (there, and it will bite here the same way):

    * Windows does not RUN a .ps1 on double-click, it opens it in Notepad.
    * This machine's execution policy blocks unsigned scripts, so a
      shortcut that merely launches PowerShell would die with a red wall of
      text about "running scripts is disabled on this system" -- which
      reads exactly like Finviz Research being broken.

  Idempotent: run it as many times as you like. WScript.Shell.CreateShortcut
  opens an existing .lnk for update, so a second run rewrites the same file
  rather than leaving 'Finviz Research - Copy' next to it.
...
.PARAMETER Name
  Shortcut name, without .lnk. Default 'Finviz Research'.

.PARAMETER Remove
  Delete the shortcut instead of creating it.

.EXAMPLE
  .\tools\install_finviz_shortcut.ps1
#>
[CmdletBinding()]
param(
  [string] $Name = 'Finviz Research',
...
# notes two pwb-toolbox checkouts exist on this machine, and the icon must
# point at the one this script was run from.
...
# -NoProfile so a slow or broken profile cannot delay or break the launch.
