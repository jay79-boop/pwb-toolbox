...
**Not every repo-shaped token is a claim that the file is here.** Three
legitimate kinds turned up on the first run: a file described in the past tense
...
# Every entry carries the reason it is here, because an allowlist without one
# becomes the place broken links go to be forgotten. The test below removes an
# entry once its reason expires, so this cannot quietly outgrow its purpose.
NOT_IN_THIS_TREE = {
    ...
    missing = sorted(
        {
            ...
            if p not in NOT_IN_THIS_TREE and not (ROOT / p).exists()
        }
    )
    assert not missing, (
        ...
        f"deliberate (a removed file, another tool's convention, a machine-local "
        f"file), add it to NOT_IN_THIS_TREE with the reason."
    )
